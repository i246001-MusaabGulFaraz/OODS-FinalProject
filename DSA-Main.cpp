...
public:
    Person() {}
    Person(string name_, string contact_): name(name_), contact(contact_) {}
    virtual ~Person() {}
    string getName() const { return name; }
    string getContact() const { return contact; }
    void setName(const string& name_) { name = name_; }
    void setContact(const string& contact_) { contact = contact_; }
};
...
public:
    Customer() {}
    Customer(string customerID_, string name_, string contact_, string address_)
        : Person(name_, contact_), customerID(customerID_), address(address_) {}
...
//@M I've made the base class here you are responsible for implementing a merge sort by priority and dates later down in the code
...
class ServiceRequest {
    string requestID;
    string vehicleReg; //Reg: Registration Number
    string customerID;
...
public:
    ServiceRequest() {}
    ServiceRequest(string requestID_, string vehicleReg_, string customerID_, string date_, string serviceType_, int priority_, double estimatedCost_)
        : requestID(requestID_), vehicleReg(vehicleReg_), customerID(customerID_), date(date_),
          serviceType(serviceType_), priority(priority_), status(Status::Pending), estimatedCost(estimatedCost_), assignedMechanicID("none") {}
    ...
    // compares based on priority -> date -> cost
    ...
    void operator=(ServiceRequest other){
...
// @H Rahim ive made the base class edit it as you see fit keep the serialize and de serialize they are needed for saving data/reading
...
public:
    static vector<string> splitLine(const string& s, char delim='|') {
        vector<string> parts;
        string store; //temperary string
        stringstream ss(s);
        while (getline(ss, store, delim)) parts.push_back(store);
        return parts;
    }
...
public:
    ...
    bool assignMechanicToRequest(const string& requestID) {
        ...
        for (auto &req : requests) {
            if (req.getID() == requestID) {
                // choose mechanic with min workload and matching specialization @M u can hard code specializations if needed
                string best = "";
    ...
    // Generate simple bill @ this is to be edited implement a customer loyalty scheme here.
    ...
    static void mergeByCost(vector<ServiceRequest>& vec, int left, int mid, int right) {
        int i, j, k; //iterators for while loops
        int n1 = mid - left + 1;
        int n2 = right - mid;

        vector<ServiceRequest> leftVec(n1), rightVec(n2);
    ...
    static void mergeSortByCost(vector<ServiceRequest>& vec, int left, int right) {
        if (left < right) {
            int mid = left + (right - left) / 2;

            mergeSortByCost(vec, left, mid);
            mergeSortByCost(vec, mid + 1, right);

            mergeByCost(vec, left, mid, right);
        }
    }

    // priority queue, orqanizes based on priority > date > cost
    vector<ServiceRequest> getTopUrgentRequests(int k) {
        priority_queue<ServiceRequest> que;
        for (const auto &r: requests) que.push(r);
        vector<ServiceRequest> out;
        for (int i=0; i<k && !que.empty(); ++i) {
            out.push_back(que.top()); que.pop();
        }
        return out;
    }
...
int main() {
    ...
    while(true) {
        ...
        if (choice == 1) {
            ...
            int k; cout << "Enter the number of top urgent requests to retrieve: "; cin >> k;
